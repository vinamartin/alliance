...
public class Client {
    ...
    public void runTests(String iorUrl, String emailAddress) throws Exception {
        ...
        SampleNsiliClient sampleNsiliClient = new SampleNsiliClient(LISTEN_PORT,
                iorUrl,
                emailAddress);
        ...
        if (hitCount > 0) {
            ...
            if (results != null && results.length > 0) {
                for (int i = 0; i < results.length; i++) {
                    ...
                    if (getAttributeFromDag(results[i], NsiliConstants.STATUS).equalsIgnoreCase(
                            NsiliCardStatus.OBSOLETE.name())) {
                        LOGGER.info("Record is {}. Not testing result",
                                NsiliCardStatus.OBSOLETE.name());
                    } else {
    ...
    /**
     * Starts the sample nsili server from the comman line and runs tests in the runTests method.
     * See README.md.
     *
     * @param args in the format `mvn -Pcorba.client -Dexec.args="url=IORURL,email=EMAIL"`, where
