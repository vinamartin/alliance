...
public class GeoBoxHandlerTest {
  ...
  public void setup() {
    geoBoxHandler =
        new GeoBoxHandler("attributeName", LAT1, LON1, LAT2, LON2, LAT3, LON3, LAT4, LON4);
  }
  ...
  public void testAcceptData() throws KlvDecodingException {
    ...
    assertThat(
        geoBoxHandler.asAttribute().get().getValue(),
        is(
            "POLYGON ((2.000000 1.000000, 4.000000 3.000000, 6.000000 5.000000, 8.000000 7.000000, 2.000000 1.000000))"));
  }
  ...
  public void testTrim() throws KlvDecodingException {
    ...
    assertThat(data.get(LAT1).get(0), is(closeTo(1, EPSILON)));
    assertThat(data.get(LON1).get(0), is(closeTo(2, EPSILON)));
    assertThat(data.get(LAT2).get(0), is(closeTo(3, EPSILON)));
    assertThat(data.get(LON2).get(0), is(closeTo(4, EPSILON)));
    assertThat(data.get(LAT3).get(0), is(closeTo(5, EPSILON)));
    assertThat(data.get(LON3).get(0), is(closeTo(6, EPSILON)));
    assertThat(data.get(LAT4).get(0), is(closeTo(7, EPSILON)));
    assertThat(data.get(LON4).get(0), is(closeTo(8, EPSILON)));
  }
  ...
  public void testGeoBoxSubsample() throws KlvDecodingException {
    ...
    for (int originalSize = start; originalSize < end; originalSize++) {
      ...
      GeoBoxHandler subsampledGeoBoxHandler = geoBoxHandler.asSubsampledHandler(subsampleCount);
  ...
  /**
   * Test the condition where the subsample count is greater than the number of data points in the
   * handler.
   */
  @Test
