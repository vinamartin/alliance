...
public class BannerMarkings implements Serializable {
    ...
    public enum ClassificationLevel {
        ...
        private static final Map<String, ClassificationLevel> LOOKUP_MAP = Arrays.stream(
                ClassificationLevel.values())
                .collect(Collectors.toMap(cl -> cl.name, cl -> cl));

        private static final Map<String, ClassificationLevel> SHORTNAME_LOOKUP = Arrays.stream(
                ClassificationLevel.values())
    ...
    enum AeaType {
        RD("RESTRICTED DATA"), FRD("FORMERLY RESTRICTED DATA");
    ...
    static class SapControl implements Serializable {
        ...
        private SapControl(String programString) {
            programs = ImmutableList.copyOf(Pattern.compile("[/]")
    ...
    static class AeaMarking implements Serializable {
        ...
        private AeaMarking(String marking) {
            ...
            if (split.length == 1) {
                ...
                cnwdi = false;
                sigmas = ImmutableList.copyOf(Pattern.compile(" ")
                        .splitAsStream(split[1].substring("SIGMA".length())
                                .trim())
    ...
    private BannerMarkings(MarkingType type, String classificationSegment, String inputMarkings)
        ...
        case US:
            ...
            if (classification == null) {
                throw new MarkingsValidationException("Unknown classification marking",
                        inputMarkings);
            }
            break;
        case FGI:
            fgiAuthority = classificationSegment.split(" ")[0];
            classification = ClassificationLevel.lookup(
                    classificationSegment.substring(fgiAuthority.length())
                            .trim());
            if (classification == null) {
                throw new MarkingsValidationException("Unknown classification marking",
                        inputMarkings);
            ...
            break;
        case JOINT:
            String suffix = classificationSegment.substring("JOINT".length())
                    .trim();

            classification = ClassificationLevel.lookup(Arrays.stream(ClassificationLevel.values())
                    .map(ClassificationLevel::getName)
                    .filter(suffix::startsWith)
                    .findFirst()
                    .orElse(null));
            if (classification == null) {
                throw new MarkingsValidationException("Unknown JOINT classification marking",
                        inputMarkings);
            }
            suffix = suffix.substring(classification.getName()
                    .length());
            jointAuthorities = ImmutableList.copyOf(Pattern.compile(" ")
    ...
    public boolean isNato() {
        return (type == MarkingType.FGI && ("COSMIC".equals(fgiAuthority) || "NATO".equals(
                fgiAuthority)));
    }
    ...
    public static BannerMarkings parseMarkings(String markings) throws MarkingsValidationException {
        ...
        bannerMarkings.jointAuthorities = ensureCollectionInitialized(
                bannerMarkings.jointAuthorities);
        bannerMarkings.usFgiCountryCodes = ensureCollectionInitialized(
                bannerMarkings.usFgiCountryCodes);
        bannerMarkings.sciControls = ensureCollectionInitialized(bannerMarkings.sciControls);
        bannerMarkings.disseminationControls = ensureCollectionInitialized(
        ...
        bannerMarkings.otherDissemControl = ensureCollectionInitialized(
    ...
    private boolean processSap(String segment) {
        if ((sapControl != null) || (!segment.startsWith("SAR-") && !segment.startsWith(
                "SPECIAL ACCESS REQUIRED-") && !segment.equals("HVSACO"))) {
            return false;
        }
    ...
    private boolean processAea(String segment) {
        if ((aeaMarking != null) || (!segment.startsWith("RD") && !segment.startsWith("FRD")
                && !segment.startsWith("RESTRICTED DATA") && !segment.startsWith(
    ...
    private boolean processUsFgi(String segment) {
        ...
        String suffix = segment.substring("FGI".length())
                .trim();
        if (suffix.isEmpty()) {
            usFgiCountryCodes = ImmutableList.of();
        } else {
            usFgiCountryCodes = ImmutableList.copyOf(Pattern.compile(" ")
                    .splitAsStream(suffix)
                    .collect(Collectors.toList()));
        }
        return true;
    ...
    private boolean processDisseminationControls(String segment) {
        ...
        if (disseminationControls != null || !(split[0].startsWith("REL TO") ||
                split[0].startsWith("DISPLAY ONLY") || Arrays.stream(DissemControl.values())
        ...
        for (String s : split) {
            if (s.startsWith("REL TO")) {
                ...
                relTo = ImmutableList.copyOf(Pattern.compile(",")
                        .splitAsStream(suffix)
                        .map(String::trim)
                        .collect(Collectors.toList()));
            } else if (s.startsWith("DISPLAY ONLY")) {
                String suffix = s.substring("DISPLAY ONLY".length());
                displayOnly = ImmutableList.copyOf(Pattern.compile(",")
                        .splitAsStream(suffix)
                        .map(String::trim)
                        .collect(Collectors.toList()));
            } else {
