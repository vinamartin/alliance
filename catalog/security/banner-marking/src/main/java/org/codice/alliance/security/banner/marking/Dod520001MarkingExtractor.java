 ...
 * <p>
 * These markings are in addition to the common set of markings as defined in our taxonomy and
 * managed by the {@link BannerCommonMarkingExtractor}.
 *
 * @see <a href="http://www.dtic.mil/whs/directives/corres/pdf/520001_vol2.pdf">DOD Manual Number 5200.01, Volume 2</a>
 */
public class Dod520001MarkingExtractor extends MarkingExtractor {
    ...
    Attribute processSap(Metacard metacard, BannerMarkings bannerMarkings) {
        ...
        return new AttributeImpl(SECURITY_DOD5200_SAP,
                ImmutableList.<String>of(sapControl.toString()));
    }
    ...
    Attribute processAea(Metacard metacard, BannerMarkings bannerMarkings) {
        ...
        return new AttributeImpl(SECURITY_DOD5200_AEA,
                ImmutableList.<String>of(aeaMarking.toString()));
    }

    Attribute processDodUcni(Metacard metacard, BannerMarkings bannerMarkings) {
        if (bannerMarkings.getDodUcni()) {
            return new AttributeImpl(SECURITY_DOD5200_DODUCNI,
    ...
    Attribute processDoeUcni(Metacard metacard, BannerMarkings bannerMarkings) {
        if (bannerMarkings.getDoeUcni()) {
            return new AttributeImpl(SECURITY_DOD5200_DOEUCNI,
    ...
    Attribute processFgi(Metacard metacard, BannerMarkings bannerMarkings) {
        ...
        String fgi = fgiCountryCodes.stream()
                .collect(Collectors.joining(" ", FGI_PREFIX, ""))
                .trim();
    ...
    Attribute processOtherDissem(Metacard metacard, BannerMarkings bannerMarkings) {
        List<String> otherDissem = bannerMarkings.getOtherDissemControl()
                .stream()
                .map(BannerMarkings.OtherDissemControl::getName)
                .collect(Collectors.toList());
        if (!CollectionUtils.isEmpty(bannerMarkings.getAccm())) {
            otherDissem.addAll(bannerMarkings.getAccm()
                    .stream()
        ...
        if (currAttr != null) {
            return new AttributeImpl(SECURITY_DOD5200_OTHER_DISSEM,
                    dedupedList(otherDissem, currAttr.getValues()));
        } else {
            return new AttributeImpl(SECURITY_DOD5200_OTHER_DISSEM,
                    ImmutableList.<String>copyOf(otherDissem));
        }
    }
