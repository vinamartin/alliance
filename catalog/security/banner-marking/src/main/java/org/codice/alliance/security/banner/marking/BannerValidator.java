...
public class BannerValidator {
    ...
    private static final Comparator<String> COUNTRY_CODE_COMPARATOR = (o1, o2) -> {
    ...
    private static final Comparator<String> USA_FIRST_COUNTRY_CODE_COMPARATOR = (o1, o2) -> {
        if (o1.equals("USA")) {
            return -1;
        }
        if (o2.equals("USA")) {
    ...
    static void validate(BannerMarkings bannerMarkings) throws MarkingsValidationException {
        ...
        if (!errors.isEmpty()) {
            throw new MarkingsValidationException("Errors found in processing",
                    bannerMarkings.getInputMarkings(), errors);
        }
    }
    ...
    private static Set<ValidationError> validateFgiJoint(BannerMarkings bannerMarkings) {
        ...
        if (bannerMarkings.getType() == FGI) {
            ...
            if (bannerMarkings.getFgiAuthority()
                    .equals("NATO") || bannerMarkings.getFgiAuthority()
                    .equals("COSMIC")) {
                if (bannerMarkings.getDisseminationControls()
                        .contains(NOFORN)) {
                    errors.add(
                            new ValidationError("No use of NOFORN with NATO documents", "4.b.3."));
                }
            }
        }
        ...
        if (bannerMarkings.getNatoQualifier() != null) {
            if (bannerMarkings.getType() != FGI) {
                errors.add(new ValidationError(
                        String.format("%s classification qualifier only valid for NATO markings",
                                bannerMarkings.getNatoQualifier()), "4.b."));
            } else {
                if (bannerMarkings.getNatoQualifier()
                        .equals("ATOMAL")) {
                    if (!(bannerMarkings.getFgiAuthority()
                            .equals("NATO") || bannerMarkings.getFgiAuthority()
                            .equals("COSMIC"))) {
                        errors.add(new ValidationError(
                                "ATOMAL classification qualifier only valid for NATO markings",
                                "4.b."));
                    }
                } else {
                    if (!bannerMarkings.getFgiAuthority()
                        ...
                        errors.add(new ValidationError(String.format(
                                ...
                                bannerMarkings.getNatoQualifier()), "4.b.2.c."));
                    }
                }
            }
        }

        if (bannerMarkings.getType() == JOINT && bannerMarkings.getJointAuthorities()
                .contains("USA")) {
            if (bannerMarkings.getClassification() == RESTRICTED) {
                errors.add(
                        new ValidationError("RESTRICTED not valid for US JOINT documents", "5.d."));
            }
        }
    ...
    private static Set<ValidationError> validateSciControls(BannerMarkings bannerMarkings) {
        ...
        if (bannerMarkings.getSciControls()
                .stream()
            ...
            if (!bannerMarkings.getSciControls()
                ...
                boolean hasRelease = !bannerMarkings.getRelTo()
                        .isEmpty();
                if (!hasRelease) {
                    hasRelease = !bannerMarkings.getDisplayOnly()
                            .isEmpty();
                }
                if (!hasRelease) {
                    hasRelease = bannerMarkings.getDisseminationControls()
                            .stream()
                            .anyMatch(s -> s == NOFORN || s == ORCON || s == RELIDO);
                }
                if (!hasRelease) {
                    errors.add(new ValidationError(
                            "SCI Markings require explicit foreign disclosure or release marking",
                            "6.c."));
                }
            }
        }
    ...
    private static Set<ValidationError> validateSapControls(BannerMarkings bannerMarkings) {
        ...
        if (bannerMarkings.getSapControl() != null) {
            if (!bannerMarkings.getSapControl()
                    .isMultiple()) {
                if (bannerMarkings.getSapControl()
                        .getPrograms()
                        .size() > 4) {
                    errors.add(new ValidationError("More than four SAPs included in document ",
                            "7.e."));
                }
            }
        }
        if (bannerMarkings.getDisseminationControls() != null
                ...
                .contains(WAIVED)) {
            if (bannerMarkings.getSapControl() == null) {
                errors.add(new ValidationError("WAIVED dissemination is only appropriate for SAPs",
                        "7.f."));
            }
        }
    ...
    private static Set<ValidationError> validateAeaMarkings(BannerMarkings bannerMarkings) {
        ...
        if (bannerMarkings.getAeaMarking() != null) {
            if (bannerMarkings.getAeaMarking()
                    .getType() == RD) {
                if (bannerMarkings.getClassification()
                        .compareTo(CONFIDENTIAL) < 0) {
                    errors.add(new ValidationError("RD data must be marked at least CONFIDENTIAL",
                            "8.a.4."));
                }
            }
            if (bannerMarkings.getAeaMarking()
                ...
                if (bannerMarkings.getClassification()
                        .compareTo(CONFIDENTIAL) < 0) {
                    errors.add(new ValidationError("FRD data must be marked at least CONFIDENTIAL",
                            "8.b.2."));
                }
            }
            if (bannerMarkings.getAeaMarking()
                ...
                if (bannerMarkings.getAeaMarking()
                        .getType() == FRD) {
                    errors.add(new ValidationError(
                            "CNWDI is a subset of RD and not applicable to FRD documents",
                            "8.c.3."));
                }
            }
            if (bannerMarkings.getAeaMarking()
                    ...
                    .anyMatch(i -> i < 1 || i > 99)) {
                errors.add(
                        new ValidationError("Valid SIGMA values are 1 to 99 inclusive", "8.d.3."));
            }
        }
    ...
    private static Set<ValidationError> validateFgi(BannerMarkings bannerMarkings) {
        ...
        if (!bannerMarkings.getUsFgiCountryCodes()
                .isEmpty()) {
            ...
            if (bannerMarkings.getClassification()
                    .compareTo(CONFIDENTIAL) < 0) {
                errors.add(new ValidationError(
                        "FGI data must be classified at a level no less than CONFIDENTIAL",
                        "9.b."));
            }
            ...
            if (!ImmutableList.copyOf(sortedFgi)
                    .equals(bannerMarkings.getUsFgiCountryCodes())) {
                errors.add(new ValidationError(
    ...
    private static Set<ValidationError> validateDisseminationControls(
        ...
        if (bannerMarkings.getDisseminationControls()
            ...
            if (bannerMarkings.getRelTo()
                    .isEmpty() && !bannerMarkings.getDisseminationControls()
                    .contains(NOFORN) && !bannerMarkings.getDisseminationControls()
                    .contains(RELIDO)) {
                errors.add(
                        new ValidationError("IMCON requires a dissemination notice", "2", "1.c."));
            }
        }
        if (bannerMarkings.getDisseminationControls()
                .contains(NOFORN)) {
            ...
            if (!bannerMarkings.getRelTo()
                    ...
                    .contains(RELIDO)) {
                errors.add(new ValidationError(
                        "NOFORN dissemination may not be mixed with REL TO or RELIDO markings", "2",
                        "2.d."));
            }
        }
        if (bannerMarkings.getDisseminationControls()
            ...
            if (bannerMarkings.getClassification() == RESTRICTED) {
                errors.add(new ValidationError(
                        "PROPIN marking not valid with RESTRICTED classification level", "2",
                        "3.b."));
            }
        }
        if (bannerMarkings.getDisseminationControls()
    ...
    private static Set<ValidationError> validateRelToDisplayOnly(BannerMarkings bannerMarkings) {
        ...
        if (!bannerMarkings.getRelTo()
            ...
            if (bannerMarkings.getRelTo()
                    .size() == 1 && bannerMarkings.getRelTo()
                    .contains("USA")) {
                errors.add(new ValidationError(
                        "REL TO USA without any other countries is not a valid marking",
                        "10.e.5."));
            }
            if (bannerMarkings.getDisseminationControls()
                    .contains(NOFORN)) {
                errors.add(new ValidationError("REL TO and NOFORN not valid in a banner marking",
                        "10.e.7."));
            }
            ...
            if (!ImmutableList.copyOf(sortedRelTo)
                    .equals(bannerMarkings.getRelTo())) {
                errors.add(new ValidationError(
        ...
        if (!bannerMarkings.getDisplayOnly()
                .isEmpty()) {
            if (bannerMarkings.getClassification()
                    .compareTo(CONFIDENTIAL) < 0) {
                errors.add(new ValidationError(
            ...
            if (bannerMarkings.getDisseminationControls() != null) {
                if (bannerMarkings.getDisseminationControls()
                        .contains(RELIDO) || bannerMarkings.getDisseminationControls()
                        .contains(NOFORN)) {
                    errors.add(new ValidationError(
                            "DISPLAY ONLY dissemination not valid with NOFORN or RELIDO",
                            "10.g.4."));
                }
            }
            ...
            if (!ImmutableList.copyOf(sortedDisplayOnly)
                    .equals(bannerMarkings.getDisplayOnly())) {
                errors.add(new ValidationError(
    ...
    private static Set<ValidationError> validateOtherDissemControls(BannerMarkings bannerMarkings) {
        ...
        if (bannerMarkings.getOtherDissemControl()
                .contains(EXDIS)) {
            if (bannerMarkings.getOtherDissemControl()
                    .contains(NODIS)) {
                errors.add(new ValidationError("EXDIS and NODIS markings cannot be combined", "3",
                        "1.d."));
            }
            if (!bannerMarkings.getRelTo()
                    .isEmpty()) {
                errors.add(new ValidationError(
                        "Documents bearing the EXDIS marking cannot be released to foreign "
                                + "goverments or international organizations", "3", "1.c."));
            }
        }
            ...
            if (!bannerMarkings.getRelTo()
                ...
                errors.add(new ValidationError(
                        "Documents bearing the NODIS marking cannot be released to foreign "
                                + "goverments or international organizations", "3", "2.d."));
            }
        }
