...
/**
 * Create a {@link org.codice.alliance.video.stream.mpegts.plugins.UpdateParent.UpdateField} based
 * on a list of {@link UpdateParentFactory.Factory} factories.
 */
public class ListUpdateFieldFactory implements UpdateParentFactory.Factory {
  ...
  public UpdateParent.UpdateField build() {
    return new UpdateFieldList(
        factories.stream().map(UpdateParentFactory.Factory::build).collect(Collectors.toList()));
  }
}
