...
public class FindChildrenStreamEndPluginTest {
    ...
    public void setup()
        ...
        FindChildrenStreamEndPlugin.Factory factory =
                mock(FindChildrenStreamEndPlugin.Factory.class);
    ...
    /**
     * It is possible (in theory) that the catalog could return irregular sized batches. We
     * want to make sure this case is handled properly when mixed with intermittent exceptions.
     * In this test, we are going to return a total of 10 metacards with exceptions intermixed.
     */
    @Test
    public void testIrregularBatching()
        ...
        when(queryResponse3.getResults()).thenReturn(Arrays.asList(result6,
                result7,
                result8,
                result9,
                result10));

        when(catalogFramework.query(any())).thenThrow(SourceUnavailableException.class)
        ...
        verify(handler, times(3)).handle(Matchers.eq(context),
                Matchers.eq(parentMetacard),
                argumentCaptor.capture());
        verify(handler).end(Matchers.eq(context), Matchers.eq(parentMetacard));

        long capturedMetacardCount = argumentCaptor.getAllValues()
                .stream()
                .mapToLong(List::size)
                .sum();
    ...
    public void testSubsequentErrors()
        ...
        verify(handler, never()).handle(Matchers.eq(context),
                Matchers.eq(parentMetacard),
                any(List.class));
    ...
    public void testHandlerThrowsRuntimeException()
        ...
        when(catalogFramework.query(any())).thenAnswer(invocationOnMock -> {
            QueryRequest queryRequest = invocationOnMock.getArgumentAt(0, QueryRequest.class);
            switch (queryRequest.getQuery()
                    .getStartIndex()) {
            case 1:
    ...
    private void testCatalogFrameworkThrows(Class exceptionClass)
        ...
        when(catalogFramework.query(any())).thenThrow(exceptionClass)
                .thenReturn(queryResponse1, queryResponse2);
    ...
    /**
     * Assert the handler calls. Assumes the catalog framework should have two successful calls. Assumes the handler should have two successful calls.
     *
     * @param queryCalls        the number of expected query calls to the catalog framework (successful and exceptional calls)
     * @param firstGoodQuery    of the captured query calls, this is the index value of the first good query (ie. an exception wasn't thrown)
     * @param secondGoodQuery   of the captured query calls, this is the index value of the second good query (ie. an exception wasn't thrown)
     * @param handlerCalls      the number of expected handler calls (successful and exceptional calls)
     * @param firstGoodHandler  of the captured handlers calls, this is the index value of the first good handler call (ie. an exception wasn't thrown)
     * @param secondGoodHandler of the captured handlers calls, this is the index value of the second good handler call (ie. an exception wasn't thrown)
     * @throws UnsupportedQueryException
     * @throws SourceUnavailableException
     * @throws FederationException
     */
    private void assertHandlerCalls(int queryCalls, int firstGoodQuery, int secondGoodQuery,
            int handlerCalls, int firstGoodHandler, int secondGoodHandler)
            throws UnsupportedQueryException, SourceUnavailableException, FederationException {
        ArgumentCaptor<List> argumentCaptor = ArgumentCaptor.forClass(List.class);

        verify(handler, times(handlerCalls)).handle(Matchers.eq(context),
                Matchers.eq(parentMetacard),
                argumentCaptor.capture());
        verify(handler).end(Matchers.eq(context), Matchers.eq(parentMetacard));
        ...
        assertThat(queryRequestCaptor.getAllValues()
                .get(firstGoodQuery)
                .getQuery()
                .getStartIndex(), is(1));
        assertThat(queryRequestCaptor.getAllValues()
                .get(firstGoodQuery)
                .getQuery()
                .getPageSize(), is(FindChildrenStreamEndPlugin.BATCH_SIZE));
        assertThat(queryRequestCaptor.getAllValues()
                .get(firstGoodQuery)
                .getQuery()
                .requestsTotalResultsCount(), is(true));
        assertThat(queryRequestCaptor.getAllValues()
                .get(firstGoodQuery)
                .getQuery()
                .getSortBy()
                .getPropertyName()
                .getPropertyName(), is(Core.METACARD_CREATED));
        assertThat(queryRequestCaptor.getAllValues()
                .get(firstGoodQuery)
                .getQuery()
                .getSortBy()
                .getSortOrder(), is(SortOrder.ASCENDING));
        ...
        assertThat(argumentCaptor.getAllValues()
                .get(firstGoodHandler)
                .get(0), is(metacard1));
        assertThat(argumentCaptor.getAllValues()
                .get(secondGoodHandler)
                .get(0), is(metacard2));
    ...
    private void setIsParentDirty(boolean value) {
        context.modifyParentOrChild(isParentDirty -> {
